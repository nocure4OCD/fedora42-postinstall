...
configure_repos() {
    ...
    local free_checksum=$(curl -s https://mirrors.rpmfusion.org/free/fedora/releases.json | jq -r --arg ver "$FEDORA_VERSION" '.releases[] | select(.version==$ver) | .checksum')
    secure_download \
        "https://mirrors.rpmfusion.org/free/fedora/rpmfusion-free-release-$FEDORA_VERSION.noarch.rpm" \
        "$TMP_DIR/rpmfusion-free.rpm" \
        "$free_checksum"
    
    local nonfree_checksum=$(curl -s https://mirrors.rpmfusion.org/nonfree/fedora/releases.json | jq -r --arg ver "$FEDORA_VERSION" '.releases[] | select(.version==$ver) | .checksum')
    secure_download \
        "https://mirrors.rpmfusion.org/nonfree/fedora/rpmfusion-nonfree-release-$FEDORA_VERSION.noarch.rpm" \
        "$TMP_DIR/rpmfusion-nonfree.rpm" \
        "$nonfree_checksum"
